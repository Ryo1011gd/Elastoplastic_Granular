...
//    char buf[1024];
    ...
    while(1){
        ...
        if(strcmp(token,endcommand)==0)break;
            ...
            ratioflag=1;
                ...
                else if(strcmp(token,"Tapper")==0){
                if(fscanf(fp,"%lf",&cyb2.tapper)!=1)goto err;
            tapperflag=1;
        }
    ...
    if(!(spaceflag && typeflag && lowerflag && upperflag && veloflag && enthalpyflag && ratioflag && tapperflag  ))return 1;
        fprintf(stderr, "line %d\n", __LINE__);
    Cyboid2 += cyb2;
    return 0;
...
// Hollow cone/frustum shell (z-axis) with inner hole defined by cyb2.ratio
// Top radius scaling given by cyb2.tapper (0: cone apex, 1: cylinder)
// Hollow cone/frustum shell (z-axis) with inner hole defined by cyb2.ratio
// Top radius scaling given by cyb2.tapper (0: cone apex, 1: cylinder)
...
for (iCyb2_t iCyb2(0); iCyb2 < cybcount2; ++iCyb2) {
    ...
    // grid
    const vec3<int> count(
        std::max(1, (int)std::round(width[0] / cyb2.space)),
        std::max(1, (int)std::round(width[1] / cyb2.space)),
        std::max(1, (int)std::round(width[2] / cyb2.space))
    );
    const vec_t spacing(width[0]/count[0], width[1]/count[1], width[2]/count[2]);

    // clamp manually (ratio between 1e-9 and <1)
    double ratio  = (double)cyb2.ratio;
    if (ratio < 1e-9) ratio = 1e-9;
    if (ratio > 0.999999) ratio = 0.999999;

    double tapper = (double)cyb2.tapper;
    if (tapper < 0.0) tapper = 0.0;
    if (tapper > 1.0) tapper = 1.0;

    const double z0 = cyb2.lower[2];
    const double z1 = cyb2.upper[2];
    const double hz = std::max(1e-12, z1 - z0);

    // base outer radii
    const double a0 = 0.5 * width[0];
    const double b0 = 0.5 * width[1];

    // floors to avoid zero division
    const double a_min = 0.25 * spacing[0];
    const double b_min = 0.25 * spacing[1];

    for (double px = cyb2.lower[0] + 0.5*spacing[0]; px <= cyb2.upper[0] - 0.49*spacing[0]; px += spacing[0]) {
        for (double py = cyb2.lower[1] + 0.5*spacing[1]; py <= cyb2.upper[1] - 0.49*spacing[1]; py += spacing[1]) {
            for (double pz = cyb2.lower[2] + 0.5*spacing[2]; pz <= cyb2.upper[2] - 0.49*spacing[2]; pz += spacing[2]) {

// height fraction t in [0,1], 0 at top, 1 at bottom
const double t = (z1 - pz) / hz;

// outer radii scale from top to bottom
const double scale = (1.0 - t) + tapper * t; 
// ↑ tapper is still "top/base radius ratio"

const double a_out = std::max(a_min, a0 * scale);
const double b_out = std::max(b_min, b0 * scale);


                // inner (hole) radii
                const double a_in  = std::max(a_min * ratio, a_out * ratio);
                const double b_in  = std::max(b_min * ratio, b_out * ratio);

                // coords relative to center
                const double x = px - center[0];
                const double y = py - center[1];

                // normalized distances
                const double ro2 = (x*x)/(a_out*a_out) + (y*y)/(b_out*b_out);
                const double ri2 = (x*x)/(a_in*a_in)   + (y*y)/(b_in*b_in);
